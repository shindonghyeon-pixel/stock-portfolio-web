...
```
*(※ 본인의 `firebaseConfig` 키 값은 기존에 쓰시던 것으로 넣어주세요!)*

---

### 2단계: `src/App.js` 전체 소스코드
GitHub의 **`src/App.js`**에 아래 코드를 그대로 덮어씌워 주세요. (앱이 켜지자마자 백그라운드에서 익명 로그인을 마친 뒤 저장을 수행하므로 권한 에러가 절대 나지 않습니다.)

```react:StockPortfolioManager:src/App.jsx
...
export default function App() {
  ...
  useEffect(() => {
    ...
    // 앱 실행 시 Firebase 익명 인증 우선 실행 (권한 거부 에러 원천 차단)
  ...
  // [저장] 버튼 로직: 배치 처리 및 30초 타임아웃 안전장치 포함
  const handleSaveToDatabase = async () => {
    ...
    const timeoutPromise = new Promise((_, reject) => 
      setTimeout(() => reject(new Error("서버 응답 시간이 초과되었습니다. Firebase 보안 규칙(Rules)이나 네트워크 연결을 확인해주세요.")), 30000)
    );
